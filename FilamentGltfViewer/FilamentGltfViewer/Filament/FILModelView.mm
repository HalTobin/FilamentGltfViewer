...
- (void)transformToUnitCubeWithOffset:(ModelOffset *)offset {
    if (!_asset) {
        return;
    }
    ...
    math::float3 translationOffset = { 0.0f, 0.0f, 0.0f };
    
    if (offset != nil) {
        translationOffset.x = offset.x.floatValue;
        translationOffset.y = offset.y.floatValue;
        translationOffset.z = offset.z.floatValue;
    }
    
    math::float3 translation = -center + translationOffset;
    
    auto transform = math::mat4f::scaling(scaleFactor) * math::mat4f::translation(translation);
    tm.setTransform(tm.getInstance(_asset->getRoot()), transform);
}

- (void)transformWithScaleFactor:(NSNumber *)scaleFactor offset:(ModelOffset *)offset {
    if (!_asset) {
        return;
    }
    ...
    auto transform = math::mat4f::scaling(scale) * math::mat4f::translation(translation);
    tm.setTransform(tm.getInstance(_asset->getRoot()), transform);
}
