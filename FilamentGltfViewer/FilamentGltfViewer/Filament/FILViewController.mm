...
- (bool)loadModel:(FilamentModel *)model {
    @try {
        [self destroyModel];
        [self createRenderablesFromModel:model];
        return true;
     }
     @catch (NSException *exception) {
         NSLog(@"Couldn't load model %@", model.url.path);
...
- (void)viewDidLoad {
    ...
    // Manually create FILModelView
        self.modelView = [[FILModelView alloc] initWithFrame:self.view.bounds];
        self.modelView.autoresizingMask =
    ...
    if (_onModelTapCallback != nil) {
        _singleTapRecognizer =
                [[UITapGestureRecognizer alloc] initWithTarget:self
                                                        action:@selector(onModelTap)];
...
- (void)createRenderablesFromModel:(FilamentModel*)model {
    _model = model;
    
    NSString *path = model.url.path;
    NSString *modelName = model.url.lastPathComponent;
    ...
    if ([modelName hasSuffix:@".glb"]) {
        [self.modelView loadModelGlb:buffer];
    } else if ([modelName hasSuffix:@".gltf"]) {
    ...
    [self.modelView transformToUnitCubeWithOffset:model.offset];
}

- (void)createLights {
    assert(self.modelView != nil);
    
    if (_scene.envSkyboxPath != nil) {
        NSString* skyboxPath = _scene.envSkyboxPath.path;
        NSData* skyboxBuffer = [NSData dataWithContentsOfFile:skyboxPath];
        
        image::Ktx1Bundle* skyboxBundle =
            new image::Ktx1Bundle(
                  static_cast<const uint8_t*>(skyboxBuffer.bytes),
                  static_cast<uint32_t>(skyboxBuffer.length)
            );
        
        _skyboxTexture = Ktx1Reader::createTexture(
               self.modelView.engine,
               skyboxBundle,
               false
        );
...
- (void)reloadModel {
    [self.modelView destroyModel];
    [self createRenderablesFromModel:_model];
}
